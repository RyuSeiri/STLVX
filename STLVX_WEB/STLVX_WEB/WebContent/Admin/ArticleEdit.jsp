...
function deleteVideo(){
	if(confirm("是否删除？")){
		...
		var req = createAjaxObj();
		req.open("post", "<%=basePath%>Article.do?method=deleteVideoByArticleId&articleId="+ encodeURI(articleId));
			req.setRequestHeader("accept", "application/json");
			req.onreadystatechange = function() {
				if (req.readyState == 4 && req.status == 200) {
					//alert(req.responseText);
...
<body>
	<div id="mainDiv">
		...
		<div id="centerDiv">
			...
			<div id="right">
				<div id="current">&nbsp;&nbsp;&nbsp;&nbsp;当前位置:文章编辑</div>
				<div id="form">
					<form action="" method="post" id="contentCommit">
						<b>文章的标题:</b><input type="text" id="title"
							value="${result[0].sTitle}" />&nbsp;<input type="hidden"
							value="${result[0].sArticleId}" id="ID" /><input type="hidden"
							id="articleKind" value="${result[0].iArticleKind}" /><input
							type="button" name="button" value="删除文章信息"
							onclick="article_delete();" />&nbsp;&nbsp;&nbsp;&nbsp;<input
							type="button" name="button" value="提交内容" onclick="article_add();" />&nbsp;&nbsp;
					        <input type=${""!=result[0].filepath ? 'button' : 'hidden'} id="videoShow" onclick="showVideo();" value="显示视频">&nbsp;&nbsp;
					        <input type=${""!=result[0].filepath ? 'button' : 'hidden'} id="delete" onclick="deleteVideo();" value="删除视频">
						   <%=htmlData%>
						<textarea name="content" id="content"
							style="width: 958px; height: 500px;"><%=htmlspecialchars(htmlData)%>${result[0].sContent}
					   </textarea>
					</form>
				</div>	
			</div>
		</div>
		<div id="videoArea" style="width: 100%;height: 100%;">
			<div style="text-align:center;position:relative;z-index: 2;">
				<video id="myVideo" controls="controls" preload="auto" style="width: 800px; height: 450px">
					<source src="${result[0].filepath}">
					<element>您的浏览器不支持视频标签。( Your browser does support the
					videotag.)</element>
				</video>
				<a href="javascript:void(0);" onclick="hideVideo();" class="button">关闭</a>
			</div>
		</div>
