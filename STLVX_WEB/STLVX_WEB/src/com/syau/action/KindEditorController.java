...
public class KindEditorController {
    ...
    public void fileUpload(HttpServletRequest request, HttpServletResponse response, @RequestParam("imgFile") MultipartFile[] imgFile)
        ...
        String savePath = application.getRealPath("/") + "tmp/";
        ...
        extMap.put("media", "swf,mp4");
        //extMap.put("file", "doc,docx,xls,xlsx,ppt,htm,html,txt,zip,rar,gz,bz2");
 
        // 最大文件大小                                     
        Double maxSize = new Double("2147483648");
       ...
       /* if (!uploadDir.exists()) {
        ...
        for(MultipartFile iFile : imgFile){
        	CommonsMultipartFile cf = (CommonsMultipartFile)iFile;
//            FileItem item = (FileItem) itr.next();
        	FileItem item = cf.getFileItem();
            String fileName = item.getName();
            if (!item.isFormField()) {
            	// 判断FileItem对象里面封装的数据是一个普通文本表单字段，还是一个文件表单字段。如果是普通文本表单字段，返回一个true否则返回一个false。
                // 检查文件大小
                ...
                try {
                    ...
                    imgPath = Util.processImg(servletContext,savePath+newFileName);//截取屏幕略缩图用
                } catch (Exception e) {
               ...
               /* msg.put("videoPath", saveUrl + newFileName);
                msg.put("imgPath", imgPath);*/
    ...
    public void fileManager(HttpServletRequest request, HttpServletResponse response)
        ...
        String rootPath = application.getRealPath("/") + "tmp/";
